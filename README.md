...
- ShoppingBascket: I use this class for patterns and also for concurrence. The patterns used in this class are the pattern singleton and the iterator pattern. For the iterator pattern, i use the two vectors on this class, and i iterate them to show on console all the objects the user is going to buy and also their prices. Then i use the singleton pattern to create two baskets in which the user will introduce the books and videogames he wants to buy; Instead of creating two objects of the type shoppingBascket, i create an instance of this class in the same class, then when i want to instantiate an object of the class shoppingBascket i call to a function that gives me the instance already created on the class; This instance is first pointing to null, when the function is call, if the instance is pointing to null then it's initialize, otherwise it gives always the same instance. Which gives me this pattern is that to objects instantiate calling the function which gives me the instance, will point the same memory space. So i give a basket for the books and other one for the videogames the user wants to buy, but the two if them will finally point to same memory space. I also use the Runnable interface in this class; This interface includes an abstract method named run, i use this method to iterate throw the vector of objects the user is going to buy and the prizes, i calculate the total sum of the objects, and in each iteration i use the method sleep of the class Thread.
...
# Pattern changes
For the last part of the practice i implemented patterns to make the class more dynamic. I've implemented the iterator, singleton and decorator pattern.

I used the decorator pattern so the user has more option at the moment of buying an object. I gived the users the option of buying books and videogames, but to give the objects more dynamism, i used this pattern so the user can select for the books, to buy one with soft cover or hard cover, each one has its cost and its characteristics. The same is applied for the videogames but for this ones i give the user the chance to buy a videogame for PC or for Xbox, as the books the videogames has its own prices and characteristics.

As in many pages as amazon, each user has a shopping basket, so to implement this function to my library i used the singleton pattern. This pattern give the chance to use different baskets for each "department", but finally the baskets are all pointing to the same memory space. To finish this part i give the users the opportunity of search for his shopping basket and then give them the election of buying all the items or return them all.

To show all the items on the shopping basket of a user i used the iterator pattern, so when a user searches for its items, i transform the vector of items in a listIterator, which grants me the opportunity to iterate in any direccion, and show the items as i want and in a more efficient way.
